...
def grid_force():
    ...
    for p in x3:
        ...
        # A00: derivative wrt r11 from fiber stretch energy f(R1) = 0.5 * k * (r11 - 1)^2
        A00 = k * (r11 - 1.0)

        # A01: derivative wrt r12 from shear energy g(R2) = 0.5 * gamma * r12^2
        A01 = gamma * r12

        # A11: derivative wrt r22 from cross-section energy h(R3)
        mu    = E * 0.4     
        lamb  = E * 0.6
        ...
        A11 = 0.0
        if r22 <= 1.0:                      
            eps = ti.log(ti.max(r22, 1e-6))
            A11 = (2.0 * mu + lamb) * eps / r22 # only 1 singular value in 2D
