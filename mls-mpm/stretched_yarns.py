...
p_mass = p_vol * p_rho
E, nu = 1.5e4, 0.2  # Much higher Young's modulus to resist plastic deformation
...
def substep():
    ...
    for p in x:  # Particle state update and scatter to grid (P2G)
        ...
        if material[p] == 1:  # yarn-like material, very stiff to prevent plastic deformation
            h = 1.0  # Higher hardening coefficient to resist permanent deformation
    ...
    for i, j in grid_m:
        if grid_m[i, j] > 0:  # No need for epsilon here
            ...
            # COMPLETELY freeze the top boundary - yarns fixed at top
            ...
            # COMPLETELY freeze the bottom boundary - yarns fixed at bottom too
            if j <= 4:  # Bottom 5 grid cells are completely frozen
                grid_v[i, j][0] = 0  # No horizontal movement
                grid_v[i, j][1] = 0  # No vertical movement
...
@ti.kernel
def reset():
    n_yarns = 7  # Number of vertical stretched yarns
    particles_per_yarn = n_particles // n_yarns
    yarn_length = 0.8  # Shorter yarns, about 80% of domain height
    yarn_thickness = 0.001  # Ultra-thin yarn-like strands
    horizontal_spacing = 0.1  # Space between yarns horizontally
    ...
    for yarn_id in range(n_yarns):
        for local_id in range(particles_per_yarn):
            ...
            # Start from near bottom (y ≈ 0.1) to near top (y ≈ 0.9)
            y_pos = 0.1 + (local_id / particles_per_yarn) * yarn_length
            
            # Horizontal position with spacing
            x_pos = 0.15 + yarn_id * horizontal_spacing
            
            # Add tiny random perturbation for thickness
            # Add slight initial displacement for visual interest
            displacement = 0.0
            if yarn_id % 2 == 0:
                # Every other yarn starts with slight curve
                displacement = ti.sin(y_pos * 3.14159 * 2) * 0.02
            
            x[i] = [
                x_pos + displacement + (ti.random() - 0.5) * yarn_thickness,
...
gui = ti.GUI("Stretched Yarns (Fixed Top & Bottom) - Taichi MLS-MPM", res=512, background_color=0xFFFFFF)
reset()
gravity[None] = [0, -1]
